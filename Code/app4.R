...
ui <- bs4DashPage(
  ...
  controlbar = bs4DashControlbar(
    ...
    bs4Card(
      ...
      br(),
      br(),
      actionButton("calcUPR", "Calculate Sum of Gross UPR", class = "btn btn-primary",
                   style = "background-color: #007bff; color: white; font-size: 12px;"),
      br(),
      br(),
      actionButton("calcDAC", "Calculate Sum of DAC", class = "btn btn-primary",
                   style = "background-color: #007bff; color: white; font-size: 12px;"),
      br(),
      br(),
  ...
  body = bs4DashBody(
    ...
    bs4TabItems(
      bs4TabItem(tabName = "dataOverview",
                 fluidRow(
                   div(
                     style = "background-color: #007bff; padding: 20px; border-radius: 10px; margin-bottom: 20px; color: white;",
                     fileInput("file1", 
                               label = tags$span("Upload Premium Data as a CSV File", style = "color: white;"),
                               accept = c(".csv")
                     ),
                     tags$p(class = "instruction-header", style = "font-weight: bold; font-size: 14px; margin-top: 20px;", "Data Upload Guidelines:"),
                     tags$ul(
...
server <- function(input, output, session) {
  ...
  output$UPRSumBox <- renderValueBox({
    ...
    valueBox(
      ...
      icon = icon("calculator", class = "fa-2x", style = "color: white;"),
      color = "primary"
    )
  })
  ...
  classWiseUPR <- eventReactive(input$calcClassWiseUPR, {
    ...
    data %>%
      ...
      mutate(`Class wise Gross UPR Sum` = scales::comma(`Class wise Gross UPR Sum`)) %>%
      mutate(`Class wise DAC Sum` = scales::comma(`Class wise DAC Sum`))  # Format numbers with commas
  })
